...
def stylish_embed(title, desc, color=0x5865F2):
    ...
    e.set_footer(text="HSL • AUTH SYSTEM", icon_url=bot.user.display_avatar.url if bot.user else None)
    return e
...
async def w_add(interaction: discord.Interaction, member: discord.Member, command_name: str):
    ...
    await interaction.response.send_message(embed=stylish_embed("✅ Whitelist Added", f"**User:** {member.mention}\n**Command:** `{command_name}`\n**Status:** Granted 🟢", 0x00FF88))
...
async def w_list(interaction: discord.Interaction):
    ...
    if not rows:
        return await interaction.followup.send(embed=stylish_embed("📭 Empty", "Koi whitelist nahi hai."))
    desc = "".join([f"<@{r[0]}> → `{r[1]}`\n" for r in rows])
...
async def create_s(interaction: discord.Interaction, username: str, password: str):
    try:
        ...
        if not app:
            return await interaction.followup.send(embed=stylish_embed("⚠️ Error", "Pehle website pe App banao!"))
        try:
            ...
            await interaction.followup.send(embed=stylish_embed("❌ Failed", "Username already exists!"))
    except Exception as e:
        print(e)
        await interaction.followup.send(f"Error: {e}") if interaction.response.is_done() else await interaction.response.send_message(f"Error: {e}")
...
async def apps_s(interaction: discord.Interaction):
    try:
        ...
        if not apps:
            return await interaction.followup.send(embed=stylish_embed("📭 No Apps", "Pehle website pe app banao!"))
        ...
        err = f"⚠️ DB Error: {e}\nCheck `apps` table exist karta hai kya Railway ke hsl.db me?"
        if interaction.response.is_done():
